...
if test "$PHP_PHDFS" != "no"; then
  ...
  export CPPFLAGS="$CPPFLAGS $INCLUDES -DHAVE_PHDFS "
    if test $(getconf LONG_BIT) == '64'  ; then
       HADOOP_DIR=" -I /opt/cloudera/parcels/CDH-5.1.0-1.cdh5.1.0.p0.53/include -L /opt/cloudera/parcels/CDH-5.1.0-1.cdh5.1.0.p0.53/lib64 -I ${HADOOP_HOME}/src/c++/libhdfs -I ${JAVA_HOME}/include -I ${JAVA_HOME}/include/linux -L${HADOOP_HOME}/c++/Linux-amd64-64/lib -lhdfs -L${JAVA_HOME}/jre/lib/amd64/server -ljvm"
    else
